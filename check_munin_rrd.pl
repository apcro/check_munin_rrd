...
use vars qw($opt_V $opt_v $opt_h $opt_w $opt_c $opt_h $opt_M $opt_d $opt_H $opt_D @opt_i $opt_o $opt_N $PROGNAME);
...
sub get_last_rrd_data {
    ...
    my $start = $last - 300; # Damn rrd ! we may get two values, the one we want and 0.0, we will focus on the firs$
    my ($rrdstart, $step, $names, $data) =  RRDs::fetch($rrdfile, "--start=$start", "--end=$last", $cf) or die "fet$
...
sub sanitize {
    my $var = shift;
    if ($opt_M eq "df") {
            if (($var !~ m/dev|LVMGROUP/) || ($var =~ m/udev/) || ($var =~ m/shm/)) {   # Get rid of non physical d$
                    $var = "";
            }
            else {
                    $var =~ s/\_/\//g;
...
sub check_parameters {
    ...
    # Check warnings and critical if not for nagios
    if (!defined($opt_N)) {
...
-N, --nagios
       Skip warning and critical limits checks. Used for perfdata collection for Nagios
EOT
}
